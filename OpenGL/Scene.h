...
protected:
	...
	virtual void configureScene()
	{
		...
		for (int i = 0; i < 1; i++)
		{
			...
			//nModel->attachTexture(texture);

			//Generate object's properties
			vec3 pos(Utility::rand32(-25, 25), Utility::rand32(-25, 25), Utility::rand32(-25, 25));
			vec3 rot(0, 0, 0);
			float scale = Utility::rand32(0.25, 1.0);
			vec3 sca(scale, scale, scale);

			//Create object
			Object* tmp = new Object(vec3(0, 0, 0), rot, vec3(1, 1, 1));
...
private:
	...
	void setupTextures()
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, sceneX * aaSampleRate, sceneY * aaSampleRate, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
...
public:
	...
	Scene(AntiAliasingOption opt, int sampleRate)
	{
		aaOpt = opt;
		aaSampleRate = sampleRate;
		sceneCam = new Camera(sceneX, sceneY, 0.1, 100.0, glm::vec3(0, 0, 5.0));
		configureScene();
		setupFBO();
		setupTextures();
		fboOkay = checkFBO();
	...
	void Render(Camera* masterCam, float timeDelta)
	{
		...
		glViewport(0, 0, sceneX * aaOpt, sceneY * aaOpt);
		...
		static GLint resXLocation = glGetUniformLocation(renderQuadShader, "resolutionX");
		static GLint resYLocation = glGetUniformLocation(renderQuadShader, "resolutionY");
		static GLint intSSAALocation = glGetUniformLocation(renderQuadShader, "enableSSAA");

		glUseProgram(renderQuadShader);
		glUniformMatrix4fv(mvpLocation, 1, GL_FALSE, glm::value_ptr(T));
		//Pass scene resolution in
		glUniform1f(resXLocation, sceneX * aaSampleRate);
		glUniform1f(resYLocation, sceneY * aaSampleRate);
		//Pass in AA option as int
		glUniform1i(intSSAALocation, int(aaOpt));
